...
func TestStartBookingCheckoutCreatesReadyIntentWithFrozenProposalPricing(t *testing.T) {
	...
	checkout, err := service.StartBookingCheckout(context.Background(), "auth0|consumer", proposal.ID)

	require.NoError(t, err)
	require.NotNil(t, checkout)
	require.NotNil(t, checkout.Intent)
	assert.Equal(t, payment.StatusCheckoutReady, checkout.Intent.Status)
	assert.Equal(t, terms, checkout.Pricing)
	assert.Same(t, checkout.Intent, intentRepository.saved)
	assert.Same(t, checkout.Intent, intentRepository.checkoutReadySaved)
